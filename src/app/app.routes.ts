...
import { isAnonymousChildGuard, isAnonymousGuard, isAuthenticatedGuard } from './accounts/guards/accounts.guard';
...
export const routes: Routes = [
  { path: 'biz/:slug', loadChildren: () => import('./business/business.routes').then(r => r.BusinessRoutes) },
  ...
  {path: 'settings', loadComponent: () => import('./personal/settings/settings.component').then(c => c.SettingsComponent),loadChildren: () => import('./personal/settings/settings.routes').then((r) => r.SettingsRoutes), canActivateChild: [isAnonymousChildGuard]},
